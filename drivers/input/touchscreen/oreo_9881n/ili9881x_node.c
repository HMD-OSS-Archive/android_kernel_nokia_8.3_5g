...
static int ilitek_debug_node_buff_control(bool open)
{
	...
	if (open) {
		...
		ilits->dbl = kzalloc(TR_BUF_LIST_SIZE * sizeof(*ilits->dbl), GFP_DMA);
		...
		for (i = 0; i < TR_BUF_LIST_SIZE; i++) {
			...
			ilits->dbl[i].data = kzalloc(TR_BUF_SIZE * sizeof(unsigned char), GFP_DMA);
...
static ssize_t ilitek_proc_get_delta_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	data = kcalloc(read_length + 1, sizeof(u8), GFP_DMA);
	if (ERR_ALLOC_MEM(data)) {
		ILI_ERR("Failed to allocate data mem\n");
		goto out;
	}

	delta = kcalloc(P5_X_DEBUG_MODE_PACKET_LENGTH, sizeof(s32), GFP_DMA);
...
static ssize_t ilitek_proc_fw_get_raw_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	rawdata = kcalloc(P5_X_DEBUG_MODE_PACKET_LENGTH, sizeof(s32), GFP_DMA);
...
static ssize_t ilitek_node_change_list_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "[Patch] 20200715 gram bist check\n");
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "[Patch] 20200724 ddi sleep in issue\n");
...
static ssize_t ilitek_proc_get_gesture_mode_data_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	if (*pos != 0)
		return 0;
	...
	size = snprintf(g_user_buf, USER_STR_BUFF * sizeof(unsigned char), "%d\n", ilits->gesture);
...
static ssize_t ilitek_proc_get_gesture_mode_data_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	int gesture = 0;
	char buf[12] = {0};
	if(size > 12)
	...
	ilits->gesture = gesture;
	ilits->gesture_record = gesture;
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	data = kcalloc(512, sizeof(u32), GFP_DMA);
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	szBuf = kcalloc(IOCTL_I2C_BUFF, sizeof(u8), GFP_DMA);
	...
	case ILITEK_IOCTL_WRAPPER_RW:
		...
		if (wrap_rlen > 0) {
			wrap_rbuf = kcalloc(IOCTL_I2C_BUFF, sizeof(u8), GFP_DMA);
...
static struct file_operations proc_gesture_fops = {
	.read = ilitek_proc_get_gesture_mode_data_read,
	.write = ilitek_proc_get_gesture_mode_data_write,
};
