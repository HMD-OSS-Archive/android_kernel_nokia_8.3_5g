...
static int hw_reset(struct fpc1020_data *fpc1020)
{
	...
	pr_info("fpc IRQ before reset %d\n", irq_gpio);
...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	...
	dev_info(dev, "fpc prepare = %d\n",(int)(fpc1020->prepared));
	mutex_lock(&fpc1020->lock);
	if (enable && !fpc1020->prepared) {
		
		fpc1020->prepared = true;
		select_pin_ctl(fpc1020, "fpc1020_reset_reset");
		dev_info(dev, "fpc after reset reset\n");
		rc = vreg_setup(fpc1020, "vcc_spi", true);
		if (rc)
			goto exit;
		dev_info(dev, "fpc vcc_spi\n");
		rc = vreg_setup(fpc1020, "vdd_io", true);
		if (rc)
			goto exit_1;
		dev_info(dev, "fpc vdd_io\n");
		rc = vreg_setup(fpc1020, "vdd_ana", true);
		if (rc)
			goto exit_2;
		dev_info(dev, "fpc vdd_ana\n");
...
#endif
	hq_register_hw_info(HWID_FP, "FPC1541_S");
	dev_info(dev, "%s: ok\n", __func__);
