...
void gpio_dump(void)
{
	...
	list_for_each_entry(gdev, &gpio_devices, list){
		//if (NULL == strstr(gdev->label, "spmi"))continue;
		chip = gdev->chip;
		if (chip->dbg_show == NULL)continue;
		pr_info("[oem][GPIO] %s, %s: %d - %d\n",
				dev_name(&gdev->dev), chip->label, gdev->base, gdev->base + gdev->ngpio - 1);
		chip->dbg_show(NULL, chip);
	}
}
EXPORT_SYMBOL_GPL(gpio_dump);
#endif
#endif	/* DEBUG_FS */
