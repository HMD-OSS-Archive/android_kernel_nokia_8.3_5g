...
static int aw8624_write_cali_lra_to_file(char cali_lra){
	struct file *fp;
	char buf[50] = {0};
	loff_t pos = 0;
	mm_segment_t fs;
...
static int aw8624_get_cali_lra_from_file(char *cali_lra){
	struct file *fp;
	struct inode *node;
	int f_size;
	char *buf;
...
#ifdef AW_CALI_STORE_EXAMPLE
	return aw8624_write_cali_lra_to_file(cali_lra);
#else
	return -EBUSY;
#endif
}
...
int aw8624_get_cali_lra_from_nvram(char *cali_lra){
	/*custom add, if success return value is 0 , else -1*/
#ifdef AW_CALI_STORE_EXAMPLE
	return aw8624_get_cali_lra_from_file(cali_lra);
#else
	return -EBUSY;
#endif
}
