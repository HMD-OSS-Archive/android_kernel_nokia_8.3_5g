...
#define CALIBRATION_F0_MAX 2410		//default 2350 +/- 60
#define CALIBRATION_F0_MIN 2290
...
static int aw8624_haptic_ram_break(struct aw8624 *aw8624, bool flag) {
	
	pr_debug("%s enter flag:%d\n", __func__,flag);
	
	if (flag == true) {
		aw8624_i2c_write(aw8624, AW8624_REG_SW_BRAKE, 0x2c);
	} else {
		aw8624_i2c_write(aw8624, AW8624_REG_SW_BRAKE, 0x08);
	}
	return 0;
}
...
static int aw8624_haptic_play_mode(struct aw8624 *aw8624, unsigned char play_mode)
{
	...
	if(play_mode == AW8624_HAPTIC_CONT_MODE){
		aw8624_haptic_ram_break(aw8624,true);
	} else {
		aw8624_haptic_ram_break(aw8624,false);
	}
	return 0;
}
...
static int aw8624_haptic_read_f0(struct aw8624 *aw8624)
{
	...
	if (!f0_reg) {
		aw8624->f0 = 0;
		pr_info("%s not get f0 because f0_reg value is 0, aw8624->f0 = 0!\n", __func__);
		return 0;
	}
...
static int aw8624_haptic_f0_calibration(struct aw8624 *aw8624)
{
	...
	if (aw8624_haptic_get_f0(aw8624)) {
		...
		aw8624_set_cali_lra_to_nvram(aw8624->cali_lra);
#endif
		/* update cali step */
		aw8624_i2c_write(aw8624,
				AW8624_REG_TRIM_LRA,
		...
		char *buf)
{
	...
	if (aw8624->f0 > CALIBRATION_F0_MAX ||
		aw8624->f0 < CALIBRATION_F0_MIN) {
		len += snprintf(buf+len, PAGE_SIZE-len, "FAILED aw8624 lra f0 = %d, out of range(%d, %d)\n",
				aw8624->f0, CALIBRATION_F0_MIN, CALIBRATION_F0_MAX);
	} else
		len += snprintf(buf+len, PAGE_SIZE-len, "PASS aw8624 lra f0 = %d\n", aw8624->f0);
...
static ssize_t aw8624_name_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;

	len += snprintf(buf+len, PAGE_SIZE-len, "AW8624\n");
	return len;
}

static ssize_t aw8624_cali_lra_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct led_classdev *to_dev = dev_get_drvdata(dev);
	struct aw8624 *aw8624= container_of(to_dev, struct aw8624, to_dev);
	...
	if (aw8624_get_cali_lra_from_nvram(&cali_lra_temp) == 0) {
		len += snprintf(buf + len, PAGE_SIZE - len, 
		"aw8624_get_cali_lra_from_nvram ok, cali_lra:%d\n", cali_lra_temp);

		aw8624->cali_lra = cali_lra_temp;
		aw8624_i2c_write(aw8624, AW8624_REG_TRIM_LRA,
		aw8624->cali_lra);
	} else { // calibration f0 if nv_read fail
		len += snprintf(buf + len, PAGE_SIZE - len, 
		"aw8624_get_cali_lra_from_nvram fail! \n");
		mutex_lock(&aw8624->lock);
		aw8624_haptic_f0_calibration(aw8624);
		mutex_unlock(&aw8624->lock);
	}
...
static ssize_t
aw8624_cali_lra_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	if (aw8624_get_cali_lra_from_nvram(&cali_lra_temp) == 0) {
		pr_info("%s aw8624_get_cali_lra_from_nvram ok!, cali_lra:%d", __func__, cali_lra_temp);

		aw8624->cali_lra = cali_lra_temp;
		aw8624_i2c_write(aw8624, AW8624_REG_TRIM_LRA,
		aw8624->cali_lra);
	} else { // calibration f0 if nv_read fail
		pr_info("%s aw8624_get_cali_lra_from_nvram err!", __func__);
		mutex_lock(&aw8624->lock);
		aw8624_haptic_f0_calibration(aw8624);
		mutex_unlock(&aw8624->lock);
	}
...
static DEVICE_ATTR(name, 0664, aw8624_name_show, NULL);
static DEVICE_ATTR(cali_lra, 0664, aw8624_cali_lra_show, aw8624_cali_lra_store);
...
static void aw8624_vibrator_work_routine(struct work_struct *work)
{
	...
	pr_debug("%s enter state %d\n", __func__, aw8624->state);
...
aw8624_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	//register hw info
	hq_register_hw_info(HWID_VIBRATOR, "AW8624");
